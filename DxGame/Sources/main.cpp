...
void Player_Initialize(void)
{
	...
	// モデルの読み込み
	pl.ModelHandle = MV1LoadModel(L"DxChara.x");

	// 影描画用の画像の読み込み
	pl.ShadowHandle = LoadGraph(L"Shadow.tga");
...
void Player_Move(VECTOR MoveVector)
{
	...
	while (CheckHitKey(KEY_INPUT_0))
	{
		DrawString(0, 0, L"CHECK", GetColor(255, 255, 255));
		VECTOR Top { 0.0f, 1000.0f, 0.0f };
		VECTOR Bottom { 0.0f, -1000.0f, 0.0f };
		auto result = MV1CollCheck_Line(stg.ModelHandle, -1, VAdd(pl.Position, Top), VAdd(pl.Position, Bottom));
		if (!result.HitFlag)
		{
			break;
		}
		DrawString(0, 32, L"HIT!", GetColor(255, 255, 255));
		// NowPos = result.HitPosition;
		NowPos = VGet(NowPos.x, result.HitPosition.y, NowPos.z);
		break;
	}
...
{
	// ステージモデルの読み込み
	stg.ModelHandle = MV1LoadModel(L"test.mqo");
